...
async def query_faceted_stats(filters: dict[str, Any]) -> dict[str, Any]:
    ...
    if statuses:
        ...
        where_clauses.append(f"s.overall_status IN ({placeholders})")
    ...
    if age is not None:
        ...
        where_clauses.append(
            ...
            f"CAST(REGEXP_REPLACE(e.minimum_age, '[^0-9]', '', 'g') AS INTEGER) <= ${idx})"
        )
        params.append(age_val)
        idx += 1
        where_clauses.append(
            f"(e.maximum_age IS NULL OR e.maximum_age = '' OR "
            f"CAST(REGEXP_REPLACE(e.maximum_age, '[^0-9]', '', 'g') AS INTEGER) >= ${idx})"
        )
        params.append(age_val)
        idx += 1
    ...
    # Get total
    total = int(await pool.fetchval("SELECT COUNT(*) FROM ctgov.studies"))
...
async def _build_funnel(pool: asyncpg.Pool, filters: dict[str, Any]) -> list[dict[str, Any]]:
    """Progressive narrowing funnel: All → Condition → +Recruiting → +Age → +Sex → +Location."""
    total = int(await pool.fetchval("SELECT COUNT(*) FROM ctgov.studies"))
    funnel = [{"stage": "All Trials", "count": total}]
    ...
    if statuses:
        ...
        row = await pool.fetchval(
            f"""SELECT COUNT(DISTINCT s.nct_id) FROM ctgov.studies s
                ...
                WHERE s.overall_status IN ({placeholders})""",
            f"%{condition.lower()}%",
            *statuses,
        )
    ...
    if age is not None:
        ...
        p_idx = 2
        if statuses:
            placeholders = ", ".join(f"${p_idx + i}" for i in range(len(statuses)))
            status_where = f" AND s.overall_status IN ({placeholders})"
        ...
        row = await pool.fetchval(
            f"""SELECT COUNT(DISTINCT s.nct_id) FROM ctgov.studies s
                ...
                WHERE (e.minimum_age IS NULL OR e.minimum_age = '' OR
                    CAST(REGEXP_REPLACE(e.minimum_age, '[^0-9]', '', 'g') AS INTEGER) <= ${p_idx})
                AND (e.maximum_age IS NULL OR e.maximum_age = '' OR
                    CAST(REGEXP_REPLACE(e.maximum_age, '[^0-9]', '', 'g') AS INTEGER) >= ${p_idx+1})
                {status_where}""",
            *base_params,
            age_val,
            age_val,
